...
def calculate_portfolio_irr_from_data(scenario_data):
    """Calculate portfolio IRR from raw scenario data - FIXED to use equity_cash_flow_pre_distributions like main.py"""
    ...
    # Use equity_cash_flow_pre_distributions like main.py does (fall back to equity_cash_flow if not present)
    equity_cf_col = 'equity_cash_flow_pre_distributions' if 'equity_cash_flow_pre_distributions' in df.columns else 'equity_cash_flow'
    ...
    if 'period_type' in df.columns:
        co_df = df[
            ...
            (df['terminal_value'] > 0) |
            (df[equity_cf_col] != 0)  # Include any non-zero equity cash flows
        ].copy()
    else:
    ...
    portfolio_cf = co_df.groupby('date')[equity_cf_col].sum().reset_index()
    portfolio_cf = portfolio_cf.rename(columns={equity_cf_col: 'equity_cash_flow'})
...
def calculate_asset_irr_from_data(scenario_data, asset_id):
    """Calculate IRR for a specific asset - FIXED to use equity_cash_flow_pre_distributions like main.py"""
    if not scenario_data:
        return None
    ...
    try:
        ...
        if equity_cf_col not in df.columns:
            print(f"      ERROR: Asset {asset_id} - '{equity_cf_col}' column missing in scenario data")
            return None
        ...
        if 'period_type' in asset_df.columns:
            co_df = asset_df[
                ...
                (asset_df[equity_cf_col] != 0)  # Include any non-zero equity cash flows
            ].copy()
        else:
            # If no period_type, include all data
            co_df = asset_df.copy()
        ...
        if equity_cf_col not in co_df.columns:
            print(f"      ERROR: Asset {asset_id_str} - '{equity_cf_col}' column missing after filtering")
            return None
        
        asset_cf = co_df[['date', equity_cf_col]].copy()
        asset_cf = asset_cf.rename(columns={equity_cf_col: 'equity_cash_flow'})
        ...
        if asset_cf.empty:
            ...
            total_cf = co_df[equity_cf_col].sum()
...
def get_base_case_metrics(unique_id=None):
    ...
    try:
        with mongo_session() as db_mgr:
            ...
            # CRITICAL FIX: Filter by unique_id if provided to ensure we get the correct base case
            if unique_id:
                base_query = {"$and": [base_query, {"unique_id": unique_id}]}
                print(f"Fetching base case metrics for unique_id: {unique_id}")
            else:
                print("ERROR: No unique_id provided for base case lookup. Cannot safely identify base case.")
                return None, {}, 0, 0, 0, {}
            
            base_data = list(main_collection.find(base_query))
            
            if not base_data:
                print(f"Warning: No base case found in main collection for unique_id: {unique_id}")
                return None, {}, 0, 0, 0, {}
...
def generate_enhanced_sensitivity_summary(sensitivity_prefix="sensitivity_results", output_format="excel"):
    ...
    try:
        with mongo_session() as db_mgr:
            ...
            # Do this BEFORE getting base case metrics so we can filter base case by unique_id
            ...
            # Get base case metrics for comparison - pass unique_id to filter correct base case
            base_case_portfolio_irr, base_case_asset_irrs, base_case_total_capex, base_case_total_revenue, base_case_portfolio_gearing, base_case_asset_metrics = get_base_case_metrics(portfolio_unique_id)
